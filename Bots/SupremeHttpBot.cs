...
namespace Dashboard1.Bots
{
    class SupremeHttpBot
    {
        ...
        static string products = ""; static string res = "";
        public static string GetProductsInfo(string keyword, string proxy, string category, string size, string color, string profile, string site, string delay, BackgroundWorker bw)
        {
            products = keyword;
            var time = DateTime.Now.ToShortDateString();
            //Creating Client connection	
            ...
            try
            {
                                    ...
                                    foreach (var re in r.Value)
                                    {
                                        ...
                                        foreach (var k in keys)
                                        {
                                            if (re["name"].ToString().ToLower().Contains(k.ToLower()))
                                            {
                                                ...
                                                var result = image + "|" + id;
                                                var pinfo = GetProductInfo(Convert.ToInt32(id), size, color, proxy);
                                                var fn = pinfo.Split('|');
                                                if(fn.Length > 1)
                                                {
                                                    var id1 = fn[0];
                                                    ...
                                                    var pcart = AddToCart(Convert.ToInt32(id), id1, styleId, profile, proxy, site);
                                                    var result1 = "";
                                                    foreach (var cook in pcart)
                                                    {
                                                        if (cook.Name == "Result")
                                                        {
                                                            result1 = cook.Value;
                                                    ...
                                                    if (result1.ToLower() != "Added To Cart".ToLower())
                                                    {
                                                        return result1;
                                                    }
                                                    else
                                                    {
                                                        ...
                                                        var pcheck = ProceedToCheckOut(pcart, profile, delay, proxy, site, bw);
                                                        return pcheck;
                                                        //playIcon.Kind = MaterialDesignThemes.Wpf.PackIconKind.Play;
                                                        //System.Diagnostics.Process.Start("https://www.supremenewyork.com/checkout");
                                                    }
                ...
                {
                    Thread.Sleep(2000);
                    res = GetProductsInfo(keyword, proxy, category,size,color,profile, site, delay,bw);
                } while (res == "Not Found" || !res.Contains("|"));
        ...
        public static string ProceedToCheckOut(IList<RestResponseCookie> cookies, string profile, string delay, string proxy, string site = "", BackgroundWorker bw = null)
        {
            try
            {
                //GetSlugStatus();
...
<doc>
    <assembly>
        <name>MahApps.Metro.IconPacks.Microns</name>
    </assembly>
    <members>
        <member name="M:MahApps.Metro.IconPacks.Converter.PackIconMicronsKindToImageConverter.GetPathData(System.Object)">
            <inheritdoc />
        </member>
        <member name="T:MahApps.Metro.IconPacks.PackIconMicrons">
            <summary>
        ...
        <member name="P:MahApps.Metro.IconPacks.PackIconMicrons.Kind">
            <summary>
            Gets or sets the icon to display.
            </summary>
        </member>
        <member name="T:MahApps.Metro.IconPacks.PackIconMicronsDataFactory">
            ******************************************
            This code is auto generated. Do not amend.
            ******************************************
        </member>
        <member name="M:MahApps.Metro.IconPacks.MicronsImageExtension.GetPathData(System.Object)">
            <inheritdoc />
        </member>
        <member name="T:MahApps.Metro.IconPacks.PackIconMicronsKind">
            ******************************************
            This code is auto generated. Do not amend.
            ******************************************
            <summary>
            List of available icons for use with <see cref="T:MahApps.Metro.IconPacks.PackIconMicrons" />.
            </summary>
            <remarks>
            The Microns icons are licensed under [CC BY-SA](<see><cref>https://creativecommons.org/licenses/by-sa/3.0/</cref></see>) licence.
            Contributions, corrections and requests can be made on GitHub <see><cref>https://github.com/stephenhutchings/microns</cref></see>.
...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     esponse response = (HttpWebResponse)request.GetResponse())
        ...
        public static string UpdateCheckOutLogs(string status, string deviceId, string slug, string message)
        {
            SqlConnection con = new SqlConnection(); con.ConnectionString = ValidateLicence.GlobalCon;
            ...
            try
            {
                if (con.State == ConnectionState.Closed)
                    con.Open();
                cmd.Connection = con;
                cmd.CommandText = "pUpdateCheckOutLogs";
        ...
        public static string GetSlugStatus(string slugId)
        {
            var time = DateTime.Now.ToShortDateString();
            //Creating Client connection	
            ...
            foreach (var item in dynJson)
            {
                if (item.Name == "status")
                {
                    if (item.Value == "queued")
                    {
                        do
                        {
                            result = GetSlugStatus(slugId);
                        } while (result.Contains("queued"));
                    }
                    else
                    {
                        result = item.Value;
            ...
            var deviceId = ValidateLicence.GetMacAddress();
            if (result.Contains("queued"))
            {
                var slug = result.Split(':')[1];
                UpdateCheckOutLogs("queued", deviceId, slug, result);
            }
            else
            {
                if (result.Contains("Thank"))
                {
                    UpdateCheckOutLogs("success", deviceId, null, result);
                }
                else
                {
                    UpdateCheckOutLogs("failed", deviceId, null, result);
                }
            }
        ...
        public static string fillDelivery(IWebDriver driver, string pr, string captcha, string delay)
        {
            ...
            try
            {
                IWebElement element = driver.FindElement(By.Id("g-recaptcha-response"));
                ((IJavaScriptExecutor)driver).ExecuteScript("document.getElementById('g-recaptcha-response').innerHTML='" + gresponse + "';", element);
                System.Threading.Thread.Sleep(1000);
                //((IJavaScriptExecutor)driver).ExecuteScript("alert(document.getElementById('g-recaptcha-response').value);", element);
                //element.SendKeys(gresponse);
                ////((IJavaScriptExecutor)driver).ExecuteScript("arguments[0].hidden = true;", element);
                //driver.FindElement(By.Id("g-recaptcha-response")).SendKeys(gresponse);
            }
            catch (Exception ex)
            {
