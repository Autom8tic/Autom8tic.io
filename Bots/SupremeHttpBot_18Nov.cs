    ...
    class SupremeHttpBot
    {
        ...
        public static string GetProductsInfo(string keyword, string proxy, string category)
        {
            ...
            try {
                ...
                {
                    Thread.Sleep(2000);
                    res = GetProductsInfo(keyword, proxy, category);
        ...
        public static string ProceedToCheckOut(IList<RestResponseCookie> cookies, string profile, string delay, string proxy, string site="")
        {
            ...
            foreach (var item in dynJson)
            {
                ...
                if(item.Name == "slug")
                {
                    ...
                    //result = GetSlugStatus(item.Value.ToString(), cookies);
                }
        ...
        public static void fillSupreme(string proxy, IList<RestResponseCookie> cookies)
        {
            ...
            if (proxy != "" && proxy != "NA")
            {
                ...
                if (plen > 2)
                {
                    ...
                    //proxy1.UseDefaultCredentials = true;
                    WebRequest.DefaultWebProxy = proxy1;
                    request.Proxy = proxy1; // new WebProxy(proxy.Split(':')[0] + ":" + proxy.Split(':')[1]);

                }
                else
                {
                    request.Proxy = new WebProxy(proxy.Split(':')[0] + ":" + proxy.Split(':')[1]);
                }
            }
            else
            {
            ...
            foreach (var cook in cookies)
            {
                ...
                {
                   // request.TryAddCookie(new System.Net.Cookie(cook.Name, cook.Value,"/checkout", "www.supremenewyork.com"));
                }
            }
